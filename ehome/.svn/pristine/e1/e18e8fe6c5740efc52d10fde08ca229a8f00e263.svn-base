...
public class AppMarryLoveController extends BaseController {
    ...
    private AjaxResult addLove(@RequestParam(required = false, defaultValue = "") Integer appUserId,
        ...
        if (ids.length > 0) {
            List<String> arrayIds = new ArrayList<String>(ids.length);
            AppUserModel currentAppUser = appUserService.selectByKey(appUserId);
            for (int i = 0; i < ids.length; i++) {
                arrayIds.add(ids[i]);
            }
            for (String loveAppUserId : arrayIds) {
                try {
                    appMarryLoveService.addLove(appUserId, Integer.parseInt(loveAppUserId));
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            es.submit(() -> {
                PushUtil.marryPush(11, arrayIds, "用户" + currentAppUser.getNickName()+"喜欢了你");
            });
