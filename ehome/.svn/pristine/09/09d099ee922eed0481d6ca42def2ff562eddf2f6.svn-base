...
public class ThirdPathShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		//第三方的登录绑定手机好码之后 写死密码了
		if (StringUtils.isNotBlank(appSysUser.getUserMobile())) {
			Md5Hash md5Hash = new Md5Hash(MD5.md5("a12345".trim()),
					appSysUser.getSalt(), 2);
			authenticationInfo = new SimpleAuthenticationInfo(loginName,
					md5Hash.toString(), ByteSource.Util.bytes(appSysUser
							.getSalt()), getName());
			return authenticationInfo;
		} else {
			authenticationInfo = new SimpleAuthenticationInfo(loginName,
					appSysUser.getUserPassword(),
