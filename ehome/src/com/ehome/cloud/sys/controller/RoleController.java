...
public class RoleController extends BaseController {
	...
	public ModelAndView getList(RoleModel roleModel,
		...
		try {
			...
			List<RoleModel> roleList = roleService.queryForList(
					request.getParameter("keyword") == null ? null : request
							.getParameter("keyword").toString().trim(),
					request.getParameter("page") == null ? 0 : Integer
							.parseInt(request.getParameter("page").toString()),
					request.getParameter("rows") == null ? 10 : Integer
							.parseInt(request.getParameter("rows").toString()),
				...
				for (RoleModel role : roleList) {
					...
					for (RoleModel roleM : listRole) {
						if (isAdmin) {
							roleDto.setIsShowButton(true);
							break;
						} else if (roleM.getId().intValue() == role.getId()
								.intValue()) {
			...
			if (CollectionUtils.isNotEmpty(roleDtos)) {
				...
				result.addObject("tablelist", JSON.toJSONString(roleDtos,
				...
				result.addObject("keyword",
						request.getParameter("keyword") == null ? null
	...
	@RequiresUser
	@RequiresPermissions("sys:role:addRole")
	...
	@RequiresUser
	@RequiresPermissions("sys:role:addAuth")
	...
	@RequiresUser
	@RequiresPermissions("sys:role:addAuth")
	public ModelAndView catAuth(HttpServletRequest request,
			HttpServletResponse response) {
		response.setContentType("text/html;charset=utf-8");
		response.setCharacterEncoding("utf-8");
		ModelAndView result = null;
